package com.zykj.landous.alipay;
...
/**
 * @author 作者 zyk
 * @version 创建时间：2015年1月14日 上午1:49:12 类说明
 */
public class Fiap {
	...
	double _coin = 0;
	protected static final JsonHttpResponseHandler res_alipay = new JsonHttpResponseHandler() {
		@Override
					...
					try {
						...
						if (retVal == ResultChecker.RESULT_CHECK_SIGN_FAILED) {
							...
							if (tradeStatus.equals("9000")) {
								...
								if (point_orderid.length() != 0) {
									// 支付赠品的邮费
									HttpUtils
											.payPointsOrder(res, point_orderid);
								} else if (order_no.length() > 0) {
									// 商品付款后调用
									HttpUtils.payOrder(res, order_no);
									HttpUtils.updateMobileDiscount(
											BeeFrameworkApp.res_update,
											order_no, "online",
	...
	String getOrderInfo(double position) {
		...
		sb.append(URLEncoder
				.encode("http://112.53.78.18:8088/shop/api/payment/alipay/notify_url.php"));
		...
		sb.append("\"");
		Log.i("alipay-url", new String(sb));
	...
	public void setPoint_orderid(String point_orderid) {
