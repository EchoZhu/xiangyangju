...
import com.zykj.landous.adapter.E1_EceiptAdapter;
import com.zykj.landous.view.MyListView;
import com.zykj.xiangyangju.R;

/**
 * @author 作者 zyk
 * @version 创建时间：2015年1月16日 上午12:05:29 类说明 已收货
 */
public class E1_EceiptActivity extends BaseActivity implements
	...
	private void init() {
		...
		adapter = new E1_EceiptAdapter(E1_EceiptActivity.this, data);
	...
	public void onRefresh(int id) {
		...
		listview.setRefreshTime();
		HttpUtils
				.getOrderList(res, "40&page=" + page + "&per_page=" + per_page);
	}
	...
	public void onLoadMore(int id) {
		...
		if (!MAX_Length) {
			per_page += 10;
			HttpUtils.getOrderList(res, "40&page=" + page + "&per_page="
					+ per_page);
		} else {
			Toast.makeText(E1_EceiptActivity.this, "您只有这么多订单",
						...
						for (int j = 0; j < array_order_list.length(); j++) {
							...
							shipping_fee += json.getDouble("shipping_fee");
							{
								...
								for (int k = 0; k < array_order_goods.length(); k++) {
									...
									goods_map.put("evaluation_state",
											json.getString("evaluation_state"));
